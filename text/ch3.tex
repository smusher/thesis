...
\section{Amber stop codon expression system}
...
We find that for wild-type Kir6.2 (WT) there is a x-fold increase in observed luminescence when coexpressed with SUR1 over background, and an x-fold increase for the C-terminally tagged Kir6.2 (WT-GFP).
There is no difference in surface expression of these constructs when ANAP is present in the culture medium.
When ANAP is incorporated at either residue 183 or 311 (F183* and W311* respectively) we see an x-fold increase in luminescence over background when coexpressed with SUR1 and with ANAP present in the culture medium.
The presence of the C-terminal GFP tag increases luminescence further for both constructs.
However, when F183* is transfected and ANAP is not present in the culture media we still see an x-fold increase in fluorescence over background, suggesting that a large proportion of the protein reaching the membrane does not have ANAP incorporated.
In contrast, when W311*-GFP is transfected with SUR1 in the presence of ANAP, we see a 10-fold increase in luminescence compared to when ANAP is not present.
