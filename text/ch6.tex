...
\subsection{SUR1 dramatically alters nucleotide inhibition, but only subtly \mccorrect{affects} nucleotide binding at Kir6.2}
...
Given that we were able to observe currents when we expressed WT-GFP in the absence of SUR1, we confirmed that when SUR1 was cotransfected with our Kir6.2 constructs we were measuring currents and fluorescence from correctly assembled K\ATP{} channels.
...
\begin{figure}[hbtp]
	...
	{\bf\mccorrect{SUR1 dramatically alters inhibition but only subtly alters binding at Kir6.2.}}
	\subref{ch6fig:nosur_atp}, \subref{ch6fig:nosur_tnpatp} Current inhibition of WT-GFP expressed alone in excised patches by ATP (\subref{ch6fig:nosur_atp}) or TNP-ATP (\subref{ch6fig:nosur_tnpatp}).
	...
	The dark grey curves are the fits to the WT-GFP+SUR1 current inhibition by ATP (\subref{ch6fig:nosur_atp}) or TNP-ATP (\subref{ch6fig:nosur_tnpatp}) and are shown for comparison.
	...
	\subref{ch6fig:tolb_inhibition_1} Inhibition by \SI{100}{\micro\Molar} tolbutamide of currents from WT-GFP expressed alone (grey) or with SUR1 (pink) or from W311*-GFP expressed with SUR1 (green).
...
\begin{figure}[hbtp]
	...
	{\bf\mccorrect{SUR1 $EC_{50}$ and $IC_{50}$ parameters.}}
	\subref{ch5fig:e179_ec50_fits} Posterior probability distributions for the estimated population $IC_{50}$ values are shown in dark pink ($IC_{50}$ for ATP inhibition) and orange ($IC_{50}$ for TNP-ATP inhibition).
	\subref{ch5fig:e179_ec50_fits_2} Posterior probability distributions for the estimated population values $IC_{50}$ or $EC_{50}$ values are shown in orange ($IC_{50}$ for TNP-ATP inhibition) or blue ($EC_{50}$ for TNP-ATP binding).
...
To measure FRET, we used an approach outlined by \textcite{clegg_18_1992} and \textcite{selvin_13_1995} whereby FRET is measured as an increase in the emission of the acceptor fluorophore (mOrange) on excitation of the donor fluorophore (GFP) (Figure \ref{ch6fig:sur_assays}).
...
We captured spectra from the membranes of whole cells (rather than unroofed membranes) to improve our signal-to-noise ratio.
...
\begin{figure}[hbtp]
	...
	{\bf\mccorrect{SUR1-mO associates with WT-GFP and W311*-GFP.}}
	...
	\subref{ch6fig:wt_gfp_mo_spectra_1}, \subref{ch6fig:w311_gfp_mo_spectra_1} Emission spectra from cells expressing WT-GFP+SUR1-mO (\subref{ch6fig:wt_gfp_mo_spectra_1}) or W311*-GFP+SUR1-mO (\subref{ch6fig:w311_gfp_mo_spectra_1}) excited by \SI{490}{\nano\metre} light (orange), with each spectra captured from a different cell.
	...
	\subref{ch6fig:wt_gfp_mo_spectra_2}, \subref{ch6fig:w311_gfp_mo_spectra_2} The emission spectra from panels \subref{ch6fig:wt_gfp_mo_spectra_1} and \subref{ch6fig:w311_gfp_mo_spectra_1} respectively with the idealised GFP peak subtracted are shown in black.
...
Despite being unable to measure currents in excised patches, we were able to detect ANAP and GFP fluorescence from unroofed membrane patches coexpressing W311*-GFP with either TMD0 or TMD0-L0.
...
\begin{figure}[hbtp]
	...
	{\bf\mccorrect{TMD0 and TMD0-LO associate with WT-GFP and W311*-GFP.}}
	\subref{ch6fig:tmd0s_surface_expression_1} Individual observations (points) and posterior probability distributions for surface expression of Kir6.2 constructs in the presence of SUR1 (green), TMD0 (yellow), TMD0-L0 (purple) or alone (red).
	...
	\subref{ch6fig:tmd0s_surface_expression_2}, \subref{ch6fig:tmd0s_surface_expression_3} Contrasts between the surface expression of WT-GFP (\subref{ch6fig:tmd0s_surface_expression_2}) or W311*-GFP (\subref{ch6fig:tmd0s_surface_expression_3}) when expressed alone, and their surface expression when coexpressed with SUR1, TMD0 or TMD0-L0.
...
\begin{figure}[hbtp]
	...
	{\bf\mccorrect{TMD0 and TMD0-LO subtly alter TNP-ATP binding to Kir6.2.}}
	\subref{ch6fig:tmd0s_unroofed_1}, \subref{ch6fig:tmd0s_unroofed_2} Fluorescence quenching of W311*-GFP expressed with TMD0 (\subref{ch6fig:tmd0s_unroofed_1}) or TMD0-L0 (\subref{ch6fig:tmd0s_unroofed_2}) by TNP-ATP in unroofed membrane patches is shown in blue.
	The lines are the median estimates and the smooth filled curves are the \SI{95}{\percent} credible intervals of the posterior probability distribution of fits to equation \ref{eq:hill} as described in the methods.
	...
	\subref{ch6fig:tmd0_atp_1} Current inhibition of WT-GFP+TMD0-L0 in excised patches by ATP is shown in dark pink.
	The lines are the median estimates and the smooth filled curves are the \SI{95}{\percent} credible intervals of the posterior probability distribution of fits to equation \ref{eq:hill} as described in the methods.
...
\begin{figure}[hbtp]
	...
	{\bf\mccorrect{TMD0 and TMD0-LO subtly alter the effect of nucleotides on Kir6.2.}}
	\subref{ch6fig:tmd0s_ec50s} Posterior probability distributions for the estimated population $IC_{50}$ or $EC_{50}$ values are shown in dark pink ($IC_{50}$ for ATP inhibition of WT-GFP) or blue ($EC_{50}$ for TNP-ATP binding to W311*-GFP in unroofed membranes).
	Each distribution is shaded with respect to its credible intervals.
...
Residue K205 of SUR1 is located in the L0 region which links TMD0 and TMD1 \mccorrect{(Figure {\ref{ch6fig:k205_1}})}.
...
We excised patches expressing W311*-GFP+SUR1-K205A or W311*-GFP+SUR1-K205E and measured current inhibition and fluorescence quenching by TNP-ATP simultaneously \mccorrect{(Figure {\ref{ch6fig:k205_2}}, {\ref{ch6fig:k205_3}}, {\ref{ch6fig:k205_4}})}.
We found that both substitutions resulted in an increased IC\textsubscript{50} for TNP-ATP inhibition and an increased EC\textsubscript{50} for TNP-ATP binding, with K205E exhibiting a more pronounced effect than K205A \mccorrect{(Figure {\ref{ch6fig:k205_5}})}.
Fitting the data to our MWC model gave parameter estimates for $K_A$ which were reduced when compared to wild-type SUR1; with the neutral mutation K205A not affecting $K_A$ quite as much as the charge reversal mutation K205E \mccorrect{(Figure {\ref{ch6fig:mwc_k205_1}}, {\ref{ch6fig:mwc_k205_2}})}.
...
\begin{figure}[hbtp]
	...
	{\bf\mccorrect{Functional effects of mutations at K205.}}
	\subref{ch6fig:k205_1} Residue K205 is shown in purple in the cryo-EM structure of K\ATP{} (PDB \#6C3P), with all four Kir6.2 and TMD0 and L0 of two SUR1 subunits shown.
...
\begin{figure}[hbtp]
	...
	{\bf\mccorrect{K205 mutations affect gating and nucleotide binding.}}
	\subref{ch6fig:mwc_k205_1} Fluorescence quenching (blue) and current inhibition (orange) from excised patches expressing W311*-GFP+SUR1-K205A (left) or W311*-GFP+SUR1-K205E (right).
