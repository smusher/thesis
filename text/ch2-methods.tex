...
Over 90\% full-length W311*-HA-GFP was produced under these conditions (Figure 1—Figure supplement 1D).
...
Co-expression with SUR1 increased the percentage of full-length W311*-HA-GFP produced (Figure 1—Figure supplement 1D).
...
\end{equation}
where $y$ represents corrected fluorescence intensity and $EC_{50}$ and $[TNPATP]$ are $\log_{10}$ values.
...
We used the brms package in R to perform a non-linear fit to equation \ref{eq:hill}.
...
Essentially, the $EC_{50}$ parameter was fit with a population parameter (\textgreek{b}) and an additional 'random' component that varied between experiments (\textgreek{delta}\textsubscript{experiment}).
