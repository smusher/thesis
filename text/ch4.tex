...
In addition, \textcite{cukras_role_2002} compared the $P_O$ calculated from noise analysis and the $P_O$ calculated by application of saturating concentrations of PIP\textsubscript{2} of a variety of K\ATP{} channel mutants, and found only a weak correlation between the two methods \cite{cukras_role_2002}.
...
Our data suggest that these findings can be reconciled with the observation that even within the framework of a concerted MWC model, where each nucleotide binding event is energetically coupled to the pore, the proportion of K\ATP{} channels which are in the open state with more than one Kir6.2 subunit which have TNP-ATP bound is essentially nil.
