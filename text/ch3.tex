...
\begin{figure}[h]
	...
	\subref{ch3fig:6baa_docking}, \subref{ch3fig:6c3p_docking}, \subref{ch3fig:6c3o_docking} Cryo-EM binding poses for ATP bound to Kir6.2 (white) from PDB \#6BAA (\subref{ch3fig:6baa_docking}), \#6C3P (\subref{ch3fig:6c3p_docking}) and \#6C30 (\subref{ch3fig:6c3o_docking}) overlayed with either the ten highest scoring computationally docked ATP poses (red) or the three highest scoring computationally docked TNP-ATP poses (orange).
	Starting poses for ATP were generated from ten different random seeds based on the cryo-EM poses.
	Starting poses for TNP-ATP were generated from eleven structures taken from the RCSB database for different proteins solved in the presence of TNP-ATP.
	The poses shown are from \#5SVQ (X-ray diffraction structure of TNP-ATP bound to the human P2X3 ion channel).
	\subref{ch3fig:6c3p_bound} Cryo-EM structure of the nucleotide binding site in the presence of ATP from PDB \#6C3P.
	\subref{ch3fig:6c3p_docked} As \subref{ch3fig:6c3p_bound} but with the three top scoring binding poses for docked TNP-ATP.
...
\begin{figure}[h]
	...
	\subref{ch3fig:western_2} Each lane from the gel containing C-terminally GFP-tagged constructs is displayed normalised to its highest intensity accompanied by the line averaged density trace.
...
We observed quenching of ANAP fluorescence from W311*-GFP+SUR1 in unroofed membranes over a concentration range of TNP-ATP similar to the range in which we observed inhibition of current in excised patches expressing W311*-GFP+SUR1 (Figure \ref{ch3fig:ec50_fits_3}, \ref{ch3fig:tnpatp_quenching_2}).
...
Our fluoresence measurements from excised patches are right-shifted with respect to our measurements for unroofed membranes (Figure \ref{ch3fig:pcf_1}), with an $EC_{50}$ value of \SIrange{76}{144}{\micro\Molar} in PCF compared to \SIrange{30}{45}{\micro\Molar}.
...
\begin{figure}[h]
	...
	The smooth filled curves are the \SI{95}{\percent} credible intervals of the posterior probability distribution of fits to equation \ref{eq:hill} as described in the methods.
	The dashed blue lines are the \SI{95}{\percent} credible intervals of the fits reported in Figure \ref{ch3fig:tnpatp_quenching_1}.
	...
	The posterior probability distribution for the $EC_{50}$ values in unroofed membranes is the same as reported in Figure \ref{ch3fig:ec50_fits_3}.
	}
\end{figure}
