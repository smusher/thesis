...
Imagine that instead of testing just one coin as in Figure \ref{ch2fig:coinflip}, we are interested in the differences between batches of coins.
...
\begin{figure}
	...
	\caption[Multilevel modelling of coinflips]{
		...
		The generative model was a binomial with each batch having a different probability of landing heads taken from a normal distribution with a mean of 0.5 and a standard deviation of 0.1.
...
where $\alpha_j$ is again our prior, but this time it has two separate parameters: one for the population average probability of landing heads up $\bar{\alpha}$, and one for the standard deviation of that probability between batches $\sigma$.
